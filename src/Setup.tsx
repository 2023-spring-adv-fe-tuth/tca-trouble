...
`;
  ...
  const handleColorClose = () => {
    ...
    const chosenPlayersCount = chosenPlayers.filter(
      (p) => p.name === selectedPlayer
    ).length;

    if (
      selectedColorsCount === 0 ||
      selectedColorsCount !== chosenPlayersCount
    ) {
      return;
    }
          ...
          <Box sx={{ display: "flex", alignItems: "flex-end", mt: 0.5 }}>
            ...
            {x.checked && (
              <FormControl sx={{ minWidth: 120 }} size="small">
                <Dialog
                  open={dialogOpen && selectedPlayer === x.name}
                  onClose={handleColorClose}
                >
                  <DialogTitle sx={{ textAlign: "center" }}>
                    Select color for {selectedPlayer}
                  </DialogTitle>
